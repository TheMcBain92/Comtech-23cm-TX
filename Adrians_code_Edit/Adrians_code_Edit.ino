...
byte encBtn = 2; // Encoder Button Pin
...
byte ky_040_dir = 2; // Clockwise = 1, Anti-clockwise = 0, Setup = 2
...
pinMode(11, OUTPUT); 
pinMode(12, OUTPUT);
pinMode(ky_040_clk,INPUT_PULLUP);
pinMode(ky_040_dt,INPUT_PULLUP);
...
lcd.setCursor(4,0);
lcd.print("By S McBain");
lcd.setCursor(3,2);
lcd.print("Callsign M5SJM");
lcd.setCursor(2,4);
...
void loop()
{
  ...
  if (ky_040_clkVal != ky_040_clkValLast){ // Compare KY-040 current value to last value. If not the same this means rotary encoder is rotating
    if (digitalRead(ky_040_dt) != ky_040_clkVal) { // Rotary encoder is rotating clockwise
      ky_040_dir = 1; // Store rotation status Clockwise
      if (encoderPosCount <= 253){ // If rotary encoder position is less than or equal to maximum encoder value (highest frequency) then increment encoder position by 1.
        encoderPosCount ++;
        // Increment encoder position variable by 1 each iteration
    ...
    { // Rotary encoder is rotating anti-clockwise
      ky_040_dir = 0; // Store rotation status as Anti-Clockwise
...
if (ky_040_dir == 1){
  ...
  } else if (ky_040_dir == 0) {
  Serial.println("Anti-clockwise");
  } else if (ky_040_dir == 2)  {
